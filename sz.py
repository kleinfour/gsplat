...
def gen_stats(base_path="."):
  table = []
  for path, _, files in os.walk(os.path.join(base_path, "efficientgaussian")):
    if 'SIBR_viewers' in path or 'Tracking-Anything-with-DEVA' in path or 'lama' in path:
      continue
    for name in files:
      ...
      with tokenize.open(filepath) as file_:
        ...
        table.append([relfilepath, line_count, tokens_per_line])
  return table
